...
def extract_valid_test(path, ext, split, valid=VALID):
    ls = []
    ...
    with open(path+TEST+ext, 'w') as f:
        for l in ls2:
            f.write(l)
...
def run_experiments():
    ...
    for ds in DATASETS:
        ...
        extract_valid_test(dstpath, '.nl', .15,valid='dev')

        file_utils.write_ntp_rule_template(rules, dstpath + 'rules.nlt')
        ...
        extract_valid_test(dstpath, '.txt', .15)
